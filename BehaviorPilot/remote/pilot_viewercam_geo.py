...
if VIEW in ("head", "head_gaze", "left_wrist", "right_wrist"):
    ...
    if idx == 0 and os.path.exists(poses_file):
        ...
        # ---- single static pose per exo (our own copy: NO segment re-aiming) ----
        # Each exo is ONE static camera aimed at `aim` (the manipulation centroid).
        # No segments: the object is clearly visible from a pose aimed where it is
        # DURING the manipulation (11770's can = 2415px there), and scoring==render
        # only holds when the camera we SCORE is the camera we RENDER. Segment
        # re-aiming broke that (scored one aim, rendered another -> blank column), so
        # we drop it. chosen carries no "segments", so capture() renders this fixed
        # pose throughout.
        cand = []
        for r in EXO_RADII:
            for a_deg in range(0, 360, 30):
                a = np.deg2rad(a_deg)
                cand.append({"angle": a_deg, "eye": aim + np.array([r * np.cos(a), r * np.sin(a), 1.3])})
        ...
        area = [0.0] * len(cand)
        for k in range(F):
            _load_frame(min(_N - 1, int((k + 0.5) / F * _N)))
            try:
            ...
            for i, c in enumerate(cand):
                eye = c["eye"]
                if not _in_frustum(eye, look, tc)[0]:
                    continue
                if _proj_area_px(eye, look, lo, hi) < MIN_PROJ_PX:
                    continue
                ...
                if not _in_frustum(eye, look, rp)[0]:
                    continue  # robot not in frame
                nvis[i] += 1
                area[i] += _proj_area_px(eye, look, lo, hi)
        ...
        for i in ranked:
            ...
            if all(min(abs(ang - u), 360 - abs(ang - u)) >= 45 for u in used_angles):
                chosen.append({"angle": ang, "score": nvis[i], "pred_vis": round(nvis[i] / F, 2),
                               "eye": cand[i]["eye"].tolist(), "look": look.tolist()})
        ...
        if chosen:
            with open(poses_file, "w") as f:
                json.dump(chosen, f)
            print("geometric placement (single-aim):",
