...
def main():
    ...
    m['code_sha256']={n:hashlib.sha256((R/n).read_bytes()).hexdigest() for n in ('dense_context.py','worker.py','randomness.py','native_bridge.py','policies.py','code/projects/v2sam_pccs/evaluation/pccs_metric.py')};(R/'manifest.json').write_text(json.dumps(m,indent=2));print('DENSE_PREPARED',flush=True)
