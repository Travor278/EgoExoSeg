...
from policies import EXPERTS,GROUPS,features
from gate_policy import GATES,scores,choose
...
manifest={'declared_at':time.time(),'source_only':True,'numpy':np.__version__,'scipy':scipy.__version__,'sklearn':sklearn.__version__,'data_hashes':{str(p.relative_to(P)):hashlib.sha256(p.read_bytes()).hexdigest() for phase in ('train','calibration') for p in (P/'runs'/phase).glob('rank*/records.jsonl')},'target_loaded_before_selection':False}
...
folds=list(GroupKFold(n_splits=4).split(np.arange(len(tr)),groups=[r['take_id'] for r in tr]))
for group in GROUPS:
...
for r in results:
    if r['gate']=='dominance':
        prior=old[r['name'].removeprefix('dominance_')]
