...
try:
    probe=json.loads((R/'failure_probe.json').read_text());assert probe['runs']['off1']['masks']!=probe['runs']['off2']['masks'];assert all(probe['runs'][n]['masks']==probe['runs']['off2']['masks'] for n in ('on1','on2','off_after'))
    gpu=subprocess.check_output(['nvidia-smi','--query-compute-apps=pid,process_name,used_memory','--format=csv,noheader'],text=True);assert not gpu.strip()
    from data_utils import read
    phase_run('exo2exo');read('exo2exo');run('repair_reference.py');run('evaluate.py');run('replay_gate.py')
