...
R2 在 rank2 第101条 knife pair 再次遇到历史候选差异，是上一轮 native dense 中已通过 OFF/OFF/ON/ON/OFF 验证具有基线重复变动的同一对象。本轮触发断言时尚未保存该次差异像素，不能直接声称它也是25像素。完整失败日志保留，已完成目标记录不重跑。R3 采用上一轮已使用的同当前候选bank协议：记录历史差异，所有原生方法共享本次同一候选；对有差异的对象在独立进程重新计算冻结 O-MaMa 外部参考，先用未变对象检查参考分数重现。该网络仅用于外部比较，不进入原生方法。如果恢复后的候选均与历史一致，则不加载参考网络。模型、阈值、源域选型不变，禁止根据恢复后的目标表现改选。最终须报告真实发生的差异/参考重算数。
