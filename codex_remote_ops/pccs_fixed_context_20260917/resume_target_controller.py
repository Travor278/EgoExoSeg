...
try:
    ...
    import hashlib
    selected=hashlib.sha256((R/'selection.json').read_bytes()).hexdigest()
    assert selected==json.loads((R/'manifest.json').read_text())['recovery']['frozen_selection_sha256']
    for phase in ('train','calibration','exo2ego'):read(phase)
    phase_run('exo2exo');read('exo2exo');run('repair_reference.py');run('evaluate.py')
    assert hashlib.sha256((R/'selection.json').read_bytes()).hexdigest()==selected
