...
try:
    if (R/'status.json').exists():raise RuntimeError('Refuse existing controller output')
    processes=subprocess.check_output(['nvidia-smi','--query-compute-apps=pid,process_name,used_memory','--format=csv,noheader'],text=True);(R/'gpu_preflight.txt').write_text(processes);assert not processes.strip(),'GPU occupied'
    run('test_union.py','unit_union');run('test_residual.py','unit_residual');run('prepare.py','prepare')
    run('offline_union.py','stage1_cached_union')
    for phase in ('smoke','screen','calibration'):
        arms(phase,RESIDUAL);check_anchors(phase);print('RESIDUAL_PHASE_COMPLETE',phase,flush=True)
