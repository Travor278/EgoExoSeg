...
def add_phase(manifest,phase,ann,cfg,reference,refphase):
    ...
    cfg=json.loads(json.dumps(cfg));cfg['models']['dinov3']['repo']=str(R/'code/third_parts/dinov3');cfg['runtime']['ports']['exo2ego']=29811
    for rank in range(4):
...
def main():
    ...
    add_phase(m,'cache_smoke',json.loads((Q/'smoke.json').read_text()),cfg,Q,'smoke')
    ...
    m['roi_contract']='Existing DINOv3 crop evidence added to original PCCS; no external network; source-only selection'
    m['frozen_fixed_context']=json.loads((R.parent/'pccs_fixed_context_20260917/selection.json').read_text())['selected']
    m['code_sha256']={n:hashlib.sha256((R/n).read_bytes()).hexdigest() for n in ('encoder_contract.py','cached_worker.py','fit_roi.py','evaluate.py','roi_views.py','roi_evidence.py','roi_policy.py','dense_context.py','worker.py','randomness.py','native_bridge.py','policies.py','code/projects/v2sam_pccs/evaluation/pccs_metric.py')};(R/'manifest.json').write_text(json.dumps(m,indent=2));print('DENSE_PREPARED',flush=True)
