...
def main():
    ...
    try:
        ...
        old_fixed={}
        if args.phase=='exo2ego':
            for p in (R.parent/'pccs_fixed_context_20260917/runs/exo2ego').glob('rank*/records.jsonl'):
                assert hashlib.sha256(p.read_bytes()).hexdigest()==json.loads((p.parent/'receipt.json').read_text())['records_sha256']
                old_fixed.update({(r['video_id'],r['obj_id']):r for r in map(json.loads,p.read_text().splitlines())})
        ...
        (out/'encoder_receipt.json').write_text(json.dumps(fingerprint(roi_matcher),indent=2))
        ...
        with path.open('a' if previous else 'w') as f:
            for bi,batch in enumerate(runner.test_dataloader):
                ...
                if args.phase in ('exo2exo','exo2ego'):
