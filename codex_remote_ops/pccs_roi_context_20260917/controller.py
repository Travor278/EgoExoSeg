...
def phase_run(phase):
    ...
    try:
        for rank in range(4):
            h=(R/f'{phase}_rank{rank}.log').open('w');logs.append(h);children.append(subprocess.Popen([sys.executable,str(R/('cached_worker.py' if phase in ('exo2exo','cache_smoke') else 'worker.py')),'--phase',phase,'--rank',str(rank)],env={**env,'CUDA_VISIBLE_DEVICES':str(rank)},cwd=R,stdout=h,stderr=subprocess.STDOUT))
...
try:
    ...
    run('test_roi.py');run('test_roi_policy.py');run('prepare.py')
    from data_utils import read
    for phase in ('smoke','cache_smoke','train','calibration'):phase_run(phase);read(phase);print('ROI_PHASE_COMPLETE',phase,flush=True)
    run('fit_roi.py');choice=json.loads((R/'selection.json').read_text())
    if choice['selected']['family']!='baseline':
        import yaml
        from prepare import add_phase
        source=R.parent/'pccs_corrected_exoexo_20260916';m=json.loads((R/'manifest.json').read_text());a=json.loads((source/'full_annotation.json').read_text());cfg=yaml.safe_load((source/'full_runtime.yaml').read_text());add_phase(m,'exo2exo',a,cfg,R.parent/'pccs_candidate_union_20260916','exo2exo');(R/'manifest.json').write_text(json.dumps(m,indent=2));phase_run('exo2exo');read('exo2exo');run('evaluate.py');phase_run('exo2ego');read('exo2ego');run('evaluate_exoego.py')
