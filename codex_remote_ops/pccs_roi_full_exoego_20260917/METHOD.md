...
双seed全量结论：每轮46515对/109253对象/295takes、同轮同候选；1.5×主方案两轮增益+2.4599/+2.4984点，2×次对照+2.5142/+2.5334点，四项95% take区间均高于零。支持既有测试集上对候选随机种子的重复性；两个seed不是两份独立场景数据，不保证每个对象都涨点。完整覆盖/指标复算与资源释放见validation.json和resource_release_seed1/2.json。当前同候选O-MaMa的新共同候选复验尚在运行，不能将历史56.6385作本轮成对参考。
