...
def native_select(metric,prediction,source_mask,object_index,original):
    ...
    if cfg['family']=='roi':
        ...
        return predict(row,metric._native_dense_model,cfg['group'],cfg['threshold'],cfg['gate'])
    if cfg.get('gate'):
        from gate_policy import scores,choose
        return choose(row,scores(row,metric._native_dense_model,cfg['group']),cfg['gate'],cfg['threshold'])
