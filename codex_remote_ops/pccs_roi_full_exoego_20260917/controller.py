...
def phase_run(phase):
    ...
    try:
        for rank in range(4):
            h=(R/f'{phase}_rank{rank}.log').open('w');logs.append(h);children.append(subprocess.Popen([sys.executable,str(R/('science_worker.py' if phase.startswith('science') else 'worker.py')),'--phase',phase,'--rank',str(rank)],env={**env,'CUDA_VISIBLE_DEVICES':str(rank)},cwd=R,stdout=h,stderr=subprocess.STDOUT))
...
def execute(seed):
    ...
    for phase in ('smoke'+str(seed),'full'+str(seed)):
        phase_run(phase);read(phase)
    env['PCCS_SUMMARY_PHASE']='full'+str(seed);run('summarize.py')
    assert hashlib.sha256((R/'selection.json').read_bytes()).hexdigest()==frozen
    (R/('gpu_after_seed'+str(seed)+'.txt')).write_text(subprocess.check_output(['nvidia-smi','--query-compute-apps=pid,process_name,used_memory','--format=csv'],text=True));status(state='complete',phase='seed'+str(seed)+'_complete',eta_seconds=0,remaining_seed=2 if seed==1 else None)
if __name__=='__main__':
    try:
        seed=int(sys.argv[1]) if len(sys.argv)>1 else 1
        gpu=subprocess.check_output(['nvidia-smi','--query-compute-apps=pid,process_name,used_memory','--format=csv,noheader'],text=True);(R/('gpu_preflight_seed'+str(seed)+'.txt')).write_text(gpu);assert not gpu.strip()
        if seed==1:
            assert not (R/'manifest.json').exists();run('test_roi.py');run('test_roi_policy.py');run('test_science.py');run('prepare.py')
            for phase in ('science_smoke','science_exo2exo','science_exo2ego'):phase_run(phase)
            run('science_evaluate.py')
        else:assert (R/'full1_results.json').exists() and not (R/'runs/full2').exists()
        execute(seed)
