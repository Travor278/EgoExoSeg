...
## 2026-09-16 补充：修复版权重与几何一致性实验

在完成pipeline和新指标审计后，改用用户发布的 corrected Visual e19／Fusion e20，单独建立Exo→Exo新基线。仍为1094对、1094对象、20个拍摄序列，所有方法共用本轮候选；下面的涨幅不减上一轮作者权重基线。

| 方法 | Frame IoU (%) | 相对修复版PCCS (点) | 95%序列bootstrap区间 |
|---|---:|---:|---|
| 修复版PCCS | 38.5257 | 0 | — |
| 原几何O-MaMa门槛0.05（次要） | 42.3595 | +3.8338 | [1.3203, 5.9842] |
| 保持纵横比O-MaMa门槛0.05（主比较） | 42.2665 | +3.7409 | [1.5792, 5.7901] |
| 两种几何一致才替换（次要） | 42.5829 | +4.0572 | [2.1297, 5.8095] |
| 旧学习判断器0.03迁移（次要） | 41.1004 | +2.5747 | [0.2836, 4.4629] |

主比较在观察本批结果前固定，支持当前基准上的平均正收益。次要区间未经多重比较校正；两几何一致方案的点估计最高，尚未证明其显著优于其他改进方法。不能把本批结果等同于所有新场景、所有随机种子都稳定涨点。

**新增原理：** native方案保持源、目标各自纵横比，最长边不超过1024、向下取14倍数，并对O-MaMa学习位置表作双线性插值，避免将外部目标一律拉伸至700×700。两几何一致方案分别执行canonical与native的固定0.05余弦门槛规则，只有两者选中同一候选才替换，否则保留PCCS。本轮该方案替换262对象，其中107改善、28变差；主方案替换350对象，其中123改善、55变差。两路匹配增加计算，但不新增训练，也不利用目标真值选择。

完整指标、方法边界与日志在`../pccs_corrected_exoexo_20260916/FINAL_REPORT.md`。四卡零跳过、exact覆盖、缓存IoU核验通过；任务于2026-09-16 01:30:53成功，占用节点0。
