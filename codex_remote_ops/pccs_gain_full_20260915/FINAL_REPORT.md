...
下一步先冻结已有1.5/2倍模型做一次新的候选随机种子复验，避免继续在已观察目标上选参数。新种子命名已预先声明，只改变候选生成随机性，所有比较共享同一新bank，并重新计算对应的外部参考；不能将种子当独立拍摄序列增加样本量。该复验尚未提交，见[冻结复验协议](../pccs_roi_seed_repeat_20260917/PLAN.md)。
