...
def main():
 ...
 from candidate_ops import stable_pair_seed,install_weighted_pool,attach_reliable_points
 ...
 try:
  ...
  if args.arm in ('weighted_pool','both'):
   install_weighted_pool(model)
   model.visual_anchor_expert.region_sampler.weighted_enabled=True;model.fusion_expert.region_sampler.weighted_enabled=True
  if args.arm in ('reliable_points','both'):attach_reliable_points(model)
