...
def main():
    ...
    try:
        ...
        prior={}
        for p in (R.parent/'pccs_roi_context_20260917/runs'/args.phase).glob('rank*/records.jsonl'):
            assert hashlib.sha256(p.read_bytes()).hexdigest()==json.loads((p.parent/'receipt.json').read_text())['records_sha256']
            prior.update({(r['video_id'],r['obj_id']):r for r in map(json.loads,p.read_text().splitlines())})
        ...
        with path.open('a' if previous else 'w') as f:
            for bi,batch in enumerate(runner.test_dataloader):
                ...
                for j,b in enumerate(rows):
                    ...
                    ref_omama=None;reference_expert=None;bank_file=None
                    prev=prior[key];identity=masks==prev['mask_sha256'] and base==prev['baseline'] and np.allclose(vals,prev['metrics'][prev['baseline']],atol=1e-7,rtol=0)
                    checked+=bool(identity);drift=not identity;drift_count+=drift
                    if args.phase=='exo2exo' and identity:ref_omama=prev['frozen_omama_reference'];reference_expert=prev['frozen_omama_expert']
                    q=ann[vid]['prompt']['first_frame_image'];q=q[0] if isinstance(q,list) else q
                    row={'video_id':vid,'obj_id':key[1],'take_id':q.split('/')[0],'baseline':base,'source_valid':bool(src.any()),'mask_sha256':masks,'evidence':evidence,'metrics':metrics,'original_reason':b['selection_reason'],'frozen_omama_reference':ref_omama,'frozen_omama_expert':reference_expert,'seed1_bank_changed':drift,'bank_file':bank_file}
                    row['candidate_seed']=seed;row['seed_namespace']='candidate-quality-v2:';row['seed1_expert_mask_match']={e:masks[e]==prev['mask_sha256'][e] for e in masks};row['seed1_baseline']=prev['baseline']
                    ...
                    roi,metadata=collect(roi_matcher,source_image,target_image,src.cpu().numpy(),{e:pred[0]['pred_masks_'+e][j].cpu().numpy() for e in ('visual','anchor','fusion')},batch_test=args.phase=='smoke' and bi==0 and j==0)
                    ...
                    for e in ('query','visual','anchor','fusion'):
                        ...
                        if e!='query':assert hashlib.sha256(binary.tobytes()).hexdigest()==masks[e]
                    bp=bankroot/(hashlib.sha256((key[0]+'|'+key[1]).encode()).hexdigest()[:28]+'.npz');np.savez_compressed(bp,**arrays);row['bank_file']=str(bp);row['roi_metadata']=metadata;row['native_pair_seconds']=native_seconds;row['gpu_peak_bytes']=torch.cuda.max_memory_allocated()
                    batch_records.append(row)
                if args.phase in ('smoke','exo2exo','exo2ego'):
