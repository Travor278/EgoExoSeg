        ...
        with path.open('a' if previous else 'w') as f:
            for bi,batch in enumerate(runner.test_dataloader):
                ...
                for j,b in enumerate(rows):
                    ...
                    row={'video_id':vid,'obj_id':key[1],'take_id':q.split('/')[0],'baseline':base,'source_valid':bool(src.any()),'mask_sha256':masks,'evidence':evidence,'metrics':metrics,'original_reason':b['selection_reason'],'frozen_omama_reference':None,'frozen_omama_expert':None,'seed1_bank_changed':True,'bank_file':None,'candidate_seed':seed,'seed_namespace':'candidate-quality-v1:','expanded_pair_metadata':ann[vid].get('candidate_meta',{})}
