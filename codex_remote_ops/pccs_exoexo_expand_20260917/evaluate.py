...
def main(phase='exo2exo'):
    ...
    summary={'seed_namespace':'candidate-quality-v1:','scope':'6534 new temporal recombinations of pre-existing approved prompts and GT; no new takes','selection_sha256':hashlib.sha256((R/'selection.json').read_bytes()).hexdigest(),'phase':phase,'coverage':'exact','selected':selection['selected'],'methods':results,'paired_comparisons':comparisons,'no_target_fitting':True,'actual_bridge_replay_exact':True,'pretrained_omama_used_in_method':False,'fixed_context_reference_complete':'frozen_fixed_context' in choices}
    if phase=='exo2exo':
