...
import numpy as np
METHODS=('baseline','learned_gate','omama_margin005','native_margin005','geometry_consensus');METRICS=('IoU','Dice','ContA','LocE')
def aggregate(rows):
...
def main():
 ...
 'annotation_sha256':spec['annotation_sha256'],'scope':'Exo2Exo full benchmark using corrected Visual e19/Fusion e20; primary native-geometry margin0.05; other methods exploratory' if args.stage=='full' else 'runtime smoke test; do not infer scientific gains from it','same_candidate_bank':True,'parameter_fitting':False}
 ...
 lines+=['','No model or threshold changes. All methods use the same freshly frozen V2-SAM candidate bank. Bootstrap resamples takes while preserving frame weighting for unequal take lengths. Exo2Exo outcomes were not used to refit the gate or choose its threshold. Earlier exploratory context/O-MaMa tests did inspect subsets of this benchmark; this is a frozen transfer evaluation, not a claim that the benchmark was never observed.']
