...
def main():
    ...
    try:
        ...
        active_images=images;images=Path(yaml.safe_load((C/'full_runtime.yaml').read_text())['data']['images'])
        ...
        rows=verified_rows(R/'runs'/args.phase/f'rank{args.rank}'/'records.jsonl');expected={(r['video_id'],r['obj_id']) for r in rows};path=out/'records.jsonl';previous=[json.loads(s) for s in path.read_text().splitlines()] if path.exists() else [];seen={(r['video_id'],r['obj_id']) for r in previous};assert len(seen)==len(previous) and seen<=expected
        ...
        assert seen==expected;receipt={'state':'complete','coverage':'exact','objects':len(seen),'records_sha256':sha(path),'same_shared_candidates':True};write(out/'receipt.json',receipt);status(sp,**receipt,eta_seconds=0)
