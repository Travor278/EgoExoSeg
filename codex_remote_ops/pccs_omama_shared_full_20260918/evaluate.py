...
def main():
    ...
    for rank in range(4):
        banks=verified_rows(R/'runs'/args.phase/f'rank{rank}'/'records.jsonl');prior=banks;new=verified_rows(R/'runs'/('reference_'+args.phase)/f'rank{rank}'/'records.jsonl');assert len(prior)==len(banks)==len(new)
    ...
    result={'phase':args.phase,'coverage':'exact','pairs':len({r['video_id'] for r in rows}),'objects':len(rows),'takes':len({r['take_id'] for r in rows}),'all_methods_share_saved_candidate_masks':True,'methods':methods,'paired_comparisons':contrasts,'pilot_is_not_full_benchmark':args.phase=='smoke1'};result['scope']='Fresh shared generation, not original full1 masks';write(R/(args.phase+'_results.json'),result);print(json.dumps(result),flush=True)
