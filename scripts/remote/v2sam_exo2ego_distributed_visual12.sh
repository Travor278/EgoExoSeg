...
SOURCE="$REPRO/V2sam/projects/v2sam_visual/configs/v2sam.py"
OVERLAY="$BASE/v2sam-visual-training-overlay-v5"
...
RUN="$EXP/runs/exo2ego-visual-official12-seed530358027-v1"
CFG="$EXP/frozen_configs/exo2ego_visual_official12_seed530358027_v1.py"
AUDIT="$EXP/audits/exo2ego_visual_official12_launch.txt"
...
for path in "$SOURCE" "$OVERLAY/tools/train.py" "$KNOWN_MM/mmengine/dist" \
  ...
  "$SAM2_DIR/sam2_hiera_large.pt" "$PY" "$TORCHRUN"; do
...
tmp_cfg=$(mktemp "$EXP/frozen_configs/.exo2ego_visual_official12.incomplete.XXXXXX")
"$PY" -I - "$SOURCE" "$tmp_cfg" "$TRAIN_JSON" "$TEST_JSON" \
  "$TRAIN_IMAGES" "$TEST_IMAGES" "$SAM2_DIR" "$RUN" "$resume_path" <<'PY'
import re
import sys
from pathlib import Path
...
replace_once(r"^        type=SAM2TrainRunner,$", f"        type=SAM2TrainRunner,\n        base_dir={sam2_dir!r},")
replace_once(r"^    sam2_folder='/path/Ego-Exo4D-Seg/data_segswap',$", f"    sam2_folder={train_images!r},")
replace_once(r"^    expression_file='/path/Ego-Exo4D-Seg/Ego2Exo_FullTrain\.json',$", f"    expression_file={train_json!r},")
replace_once(r"^    sam2_folder='/path/Ego-Exo4D-Seg/data_segswap',$", f"    sam2_folder={test_images!r},")
replace_once(r"^    expression_file='/path/Ego-Exo4D-Seg/ego2exo_val_framelevel\.json',.*$", f"    expression_file={test_json!r},")
...
assert abs(float(cfg.param_scheduler[0].end) - 0.6) < 1e-9
assert abs(float(cfg.param_scheduler[1].begin) - 0.6) < 1e-9
assert float(cfg.param_scheduler[1].end) == 12.0
...
lines = [
    ...
    "BATCH=16 ACCUM=4 LR=4e-5 EPOCHS=12",
    "SCHEDULER=LinearLR[0,0.6]+CosineAnnealingLR[0.6,12]",
...
cd "$REPRO"
exec "$TORCHRUN" --nproc_per_node=4 --master_port=29667 \
